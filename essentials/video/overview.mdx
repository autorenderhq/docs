...
| **Time**      | `st_{sec}`       | `st_5`        | Start time (seconds) |
...
- **GIF**: Combine `f_gif` with `st_` and `d_` to control which part of the video becomes the GIF.
- **Trim**: Use `st_` and `d_` for short clips to reduce size and processing time.
